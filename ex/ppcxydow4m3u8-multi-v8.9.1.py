...
'''
通用版 m3u8 下载,仅依赖 python3 环境
修改记录
-- 2018-04-19 同步ios 8.9.1版
...
def download_callback(status, result):
    ...
    if status:
        if pool.cancel:
            print("正在回收资源：%02d" % pool.q.qsize())
        elif pool.q.qsize() != 0:
            print("剩余下载数：%04d" % pool.q.qsize())
...
def download_m3u8(m3u8_url):
    ...
    sum_ts = len(open(m3u8_file, 'r', encoding='utf-8').readlines())
    print('资源解析完毕，总数: %d' % (sum_ts))
    ...
    result_file_name = input('处理完毕，是否修改文件名？(不修改直接确定)：\n')
...
def main():
    ...
    url = input('请输入m3u8资源地址：\n')
    pool_size = input('请输入线程数，不输入默认5线程：\n')
    if pool_size == '':
        pool_size = 5
    ...
    if new_file_name != '':
        print('================================')
        print('下载完毕，存储为 ~/ppcxy_download/m3u8目录\n文件名:')
...
if __name__ == "__main__":
    if len(sys.argv) >= 2 and sys.argv[1] != '':
        ...
        rm_file(tem_ts_path)
        rm_file(logfile)
        join_file_name = input('处理完毕，是否修改文件名？(不修改直接确定)：\n')
        if join_file_name != '':
